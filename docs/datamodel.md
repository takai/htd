...
| `created_at` | datetime | yes | **no** | Creation timestamp (RFC 3339) |
| `updated_at` | datetime | yes | yes | Last modification timestamp (RFC 3339) |
| `tags` | list of strings | no | yes | Arbitrary tags for filtering and INDEX grouping (see §3.3) |

The Markdown body stores the reference content. The `htd reference` CLI surface assumes the body opens with a one-line fact (used as the INDEX.md description) optionally followed by a `## How to apply` section. This shape is a convention, not a schema constraint — the CLI does not parse or validate it beyond extracting the first non-blank line.

### 3.2 Tool-Scoped Layout

Active references live under `reference/<tool>/` so multi-assistant repositories do not collide. `<tool>` is a flat namespace — there is no per-type subdirectory; the type taxonomy is encoded as a tag convention instead (see §3.3). The tool directory is created lazily on the first `htd reference add --tool <tool>`.
...
- **No generated index files** — All state is derived from the file system. **Exception:** `reference/<tool>/INDEX.md` is regenerated by `htd reference` commands on every mutation; see §3.4. The file is still tracked by Git so AI sessions can read it without invoking `htd`.
